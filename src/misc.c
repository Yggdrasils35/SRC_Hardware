...
void shoot_on(u32 value)
{
	...
	val = -5.1089*pow(value, 2)/10000 + 2.9534*value/10 - 2.1388;
	// val = 1.4591*pow(value, 2) / 10000 + 0.0988*value + 9.6996;
...
//	value = shoot_power_curve_a + (shoot_power_curve_b - shoot_power_curve_a)* value / MAX_SHOT_STRENGTH;
	...
	value = 5*MAX_SHOT_STRENGTH - value;
...
void chip_on(u32 value)
{
	...
	val = 1.1029 * pow(value, 2)/1000 + 8.8439*value/100 + 15.823;
	
	// val = 0.2*pow(value,2)+....   biaoding
...
void init_shoot_chip_timer(void)
{
	...
	prescale = (u16) (FREQ_PCLK_2 * 2 / (SHOOT_TIM_CLK_FRQ*5)) - 1;

	/* Time base configuration */
	TIM_TimeBaseStructure.TIM_Period = MAX_SHOT_STRENGTH*5;
